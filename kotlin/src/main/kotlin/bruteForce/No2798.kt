 ...
 * No.2798 블랙잭
 *
 * 문제 - 김정인 버전의 블랙잭에서 각 카드에는 양의 정수가 쓰여 있다. 그 다음, 딜러는 N장의 카드를 모두 숫자가 보이도록 바닥에 놓는다.
...
fun main(args: Array<String>){
    ...
    print(getCalc(numList).poll())
}

fun getCalc(numList: IntArray): PriorityQueue<Int>{
    ...
    for (i in 0 until n){
        for (j in i+1 until n){
            for (k in j+1 until n){
                val num = numList[i] + numList[j] + numList[k]
                if (num == m){
